 ...
 * @author Andre
 */
public class VerticeCasoUso extends Vertice{
	/**
	...
	public VerticeCasoUso(int codigo, int x, int y, int largura, int altura)
	{
	 ...
	 */
	public VerticeCasoUso(int codigo, int x, int y, int largura, int altura, int larguraBorda)
	{
		super(codigo, x, y, largura, altura);
	 ...
	 * @param altura o comprimento da altura do v�rtice
	 * @param larguraBorda a largura, em pixels, da borda do v�rtice
         * @param rotulo do caso de uso
	 * @see diagram.Grafo#getVertice
	 */        
	public VerticeCasoUso(int codigo, int x, int y, int largura, int altura, int larguraBorda,String nome)
	{
		super(codigo, x, y, largura, altura);
		setLarguraBorda(larguraBorda);
		setCorBorda(CORBORDAPADRAO);
		setCorFundo(CORFUNDOPADRAO);
                ...
                this.setDescricao("Caso de uso - "+nome);
	}        
...
//	M�todos SET...
	...
	public void desenharVertice(Graphics desenho, Component componente)
	{
		...
		desenho.fillOval(getX(), getY()+2, getLargura(), getAltura());
		
		//Desenha o fundo do v�rtice...
		desenho.setColor(corFundo);
		desenho.fillOval(getX() + larguraBorda, getY()+2 + larguraBorda, getLargura() - larguraBorda * 2, getAltura() - larguraBorda * 2);
	...
	{
		Ellipse2D.Float area = new Ellipse2D.Float (getX(), getY(), getLargura(), getAltura()/2);
